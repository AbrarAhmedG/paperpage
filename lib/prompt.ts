...
- Map each drawn region to the closest element type: a large photo/picture box (often with a mountain/landscape or X-through-box icon) -> "image" (put any label in "alt"); a small circle or round avatar/icon -> "image" with alt "avatar" or "icon" so it is rendered small, NOT as a full photo; a media box with a play triangle -> "video"; a horizontal row of tabs, steps, breadcrumbs, or numbered pagination (boxes labeled 1 2 3 4, arrows) -> "tabs" (put each label/number in "items"); a search or input field -> "input"; a menu or bullet list -> "list" (labels in "items"); a block of body text (several stacked lines) -> "paragraph"; a logo/brand mark -> "logo"; a divider line -> "divider".
