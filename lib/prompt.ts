  ...
  "sections": [
    {
      ...
      "elements": [
        { "type": "<one of: ${ELEMENT_TYPES.join(', ')}>", "text": "<optional>", "level": "<optional 1-4>", "variant": "<optional: primary|secondary|ghost>", "items": ["<optional list>"], "placeholder": "<optional>", "alt": "<optional>", "label": "<optional: a stat's caption or a quote's attribution>", "col": "<optional 1-indexed column>", "colSpan": "<optional integer>", "row": "<optional 1-indexed row>" }
      ]
    }
  ]
}
...
- A stack of labeled input boxes with a send/submit button (a contact or signup form) -> ONE "form" element: each field label in "items" (e.g. ["Name","Email","Message"]) and the submit button's label in "text". Use section role "contact" for a contact-form region. Do NOT emit the fields as separate "input" elements or the submit as a separate "button".
- A quotation (speech bubble, big quote marks, a testimonial card with a person's name) -> "quote": the quoted sentence in "text", the attribution (name, role) in "label". A row of such cards -> section role "testimonials" with one "quote" (plus optional avatar "image") per column.
- A large number with a small caption ("500+", "99%", "12 yrs") -> "stat": the number in "text", the caption in "label". A row of these -> section role "stats" with one stat per column.
